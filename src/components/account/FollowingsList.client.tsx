...
import { userControllerGetFollowings } from "@/api";
...
type FollowingsListClientProps = {
  initUsers: UserList;
  initPage: number;
  initTotal: number;
  id: string;
...
export function FollowingsListClient({
  initUsers,
  initPage,
  initTotal,
  id,
  pageSize = 10,
}: FollowingsListClientProps) {
  ...
  const loadMoreUsers = useCallback(async () => {
    ...
    try {
      const response = await userControllerGetFollowings({
        path: { id },
        query: {
          page,
          limit: pageSize,
      ...
      console.error("Failed to load more followings:", loadError);
