...
type RecommendUserListClientProps = {
  initUsers: UserList;
  initPage: number;
  initTotal: number;
  pageSize?: number;
...
export function RecommendUserListClient({
  initUsers,
  initPage,
  initTotal,
  pageSize = 10,
  cacheKey,
}: RecommendUserListClientProps) {
  const t = useTranslations("userList");
  const pathname = usePathname();
  const storageKey = cacheKey || `recommend-user-list-${pathname}`;
  ...
  const getInitialState = () => {
    ...
    if (navigationType === "reload") {
      try {
        ...
        console.error("Failed to clear recommend cache on refresh:", error);
      }
    ...
    try {
      ...
      console.error("Failed to restore recommend cache:", error);
    }
  ...
  useEffect(() => {
    ...
    const refetchFirstPage = async () => {
      ...
      try {
        ...
        console.error("Failed to refresh recommend users:", loadError);
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Failed to cache recommend state:", cacheError);
    }
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      try {
        ...
        console.error("Failed to save recommend scroll position:", scrollError);
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error("Failed to restore recommend scroll position:", restoreError);
      setScrollRestored(true);
    }
  ...
  const loadMoreUsers = useCallback(async () => {
    ...
    try {
      ...
      console.error("Failed to load more recommend users:", loadError);
      setError(t("loadFailed"));
    } finally {
      setLoading(false);
    }
  }, [fetchPage, hasMore, initTotal, loading, page, t, users.length]);
