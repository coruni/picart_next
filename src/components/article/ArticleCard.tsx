        ...
        return (
          <div className="mt-3 flex gap-2">
            {article.images.slice(0, 2).map((img, idx) => (
              <div
                key={idx}
                className="rounded-xl overflow-hidden w-31/100 relative aspect-square"
              >
                <Image
                  src={img}
      ...
      return (
        <div className="mt-3 flex gap-2">
          {displayImages.map((img, idx) => (
            <div
              key={idx}
              className="rounded-xl overflow-hidden w-1/5 relative aspect-square inline-block"
            >
              <Image
                src={img}
                alt={`${article?.title} ${idx + 1}`}
    ...
    <article className="p-6 border-border border-b">
      ...
      <section className="cursor-pointer">
        <Link href={`/article/${article.id}`}>
          <h3 className="mt-2 font-bold hover:text-primary">
            {article?.title}
          </h3>
          {summaryHtml && (
            <p
              className="article-summary-html mt-1 px-0! text-secondary text-sm leading-5 line-clamp-2 overflow-hidden"
